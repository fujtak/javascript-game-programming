...
const scale = (Math.PI * 2 * nWaves) / maxDistance
// const scale = 0.37
...
const processImg = loaded => {
  ...
  for(let y = 0; y < 300; ++y) {
    for(let x = 0; x < 300; ++x) {
      const cx = x - 150
      const cy = y - 150
      // 中心からの距離と関係づけたsinカーブで波紋にする
      const a = Math.sin(Math.sqrt(cx * cx + cy * cy) * scale) * angleRadian
      // const a = Math.sin(Math.sqrt(cx * cx + cy * cy) * scale) * amplitude
      const c = Math.cos(a)
      const s = Math.sin(a)
